...
def create_logn_costs(
    ...
    """
    Fills a row of the Cost matrix based on a log n function.
    ...
    if verbose:
        print(f'[{index}] - Creating quadratic costs with f(x) =' +
              ...
              f'. RNG seed = {rng_seed}')
    # Fills row in the matrix
    matrix[index][:] = [alpha + beta*np.log(x+1)
                        for x in range(tau+1)]
...
def create_random_costs(
    ...
    """
    Fills a row of the Cost matrix based on a values from a uniform distribution.
    ...
    np.random.seed(rng_seed)
    # Generates random values
    values = np.random.uniform(0, tau, tau+1)
    if verbose:
